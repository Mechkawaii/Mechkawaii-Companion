(function () {
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      @keyframes mkwHealGlow {
        0% { box-shadow: 0 0 0 rgba(70,255,145,0); }
        35% { box-shadow: 0 0 0 3px rgba(70,255,145,.35), 0 0 28px rgba(70,255,145,.85); }
        100% { box-shadow: 0 0 0 rgba(70,255,145,0); }
      }
      @keyframes mkwHeartPulse {
        0%, 100% { transform: scale(1); filter: drop-shadow(0 0 0 rgba(70,255,145,0)); }
        45% { transform: scale(1.28); filter: drop-shadow(0 0 10px rgba(70,255,145,.95)); }
      }
      ...
      .mkw-resource-title { font-weight:950; font-size:19px; margin-bottom:6px; }
      .mkw-resource-subtitle { color:rgba(255,255,255,.72); font-size:13px; line-height:1.35; margin-bottom:14px; }
      ...
      .mkw-resource-target:not(:disabled):hover { background:rgba(255,255,255,.1); border-color:rgba(255,210,77,.45); }
      .mkw-resource-target:disabled { opacity:.42; filter:grayscale(.75); cursor:not-allowed; }
      .mkw-resource-portrait { width:48px; height:48px; object-fit:contain; border-radius:12px; background:rgba(255,255,255,.08); flex:0 0 auto; }
      .mkw-resource-info { flex:1; min-width:0; }
      .mkw-resource-name { font-weight:950; }
      .mkw-resource-class { font-size:12px; color:rgba(255,255,255,.62); margin-top:2px; }
      .mkw-resource-value { font-weight:950; color:#ffd24d; white-space:nowrap; }
      .mkw-resource-badge { font-size:10px; font-weight:950; color:#111; background:#cfd3d8; border-radius:999px; padding:3px 7px; white-space:nowrap; margin-top:5px; }
      ...
      .mkw-resource-danger { border-color:rgba(255,105,120,.5); background:rgba(255,80,100,.12); }
    `;
  ...
  function refreshKeyButtons(){
    qsa("#repairKeysDisplay .key-button, #repairKeysDisplay button").forEach(btn => {
      const active = btn.dataset.active !== "false";
      btn.style.backgroundImage = `url('${active ? ICONS.keyOn : ICONS.keyOff}')`;
      btn.classList.toggle("is-on", active);
    });
  }

  function refreshShieldButtons(){
    qsa("#shieldsDisplay .key-button, #shieldsDisplay button").forEach(btn => {
      const active = btn.dataset.active !== "false";
      btn.style.backgroundImage = `url('${active ? ICONS.shieldOn : ICONS.shieldOff}')`;
      btn.classList.toggle("is-on", active);
    });
  }
  ...
  function updateCurrentHpUI(char, hpCur){
    ...
    const heart = hearts[Math.max(0, hpCur - 1)];
    if (hpCard) {
    ...
    if (heart) {
      heart.classList.remove("mkw-heart-pulse");
      void heart.offsetWidth;
      heart.classList.add("mkw-heart-pulse");
      setTimeout(() => heart.classList.remove("mkw-heart-pulse"), 850);
    }
  }
  ...
  async function openRepairModal(index, btn){
    ...
    team.forEach(char => {
      ...
      target.innerHTML = `
        ...
        <div class="mkw-resource-info">
          <div class="mkw-resource-name">${getName(char, lang)}</div>
          <div class="mkw-resource-class">${getClass(char, lang)}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;">
          <div class="mkw-resource-value">${cur}/${max}</div>
          ${isMax ? `<div class="mkw-resource-badge">${lang === "fr" ? "PV max" : "Max HP"}</div>` : ""}
        </div>
      `;
  ...
  async function openShieldModal(index, btn){
    ...
    const { modal, panel } = openBaseModal(
      isAvailable ? (lang === "fr" ? "Assigner un bouclier" : "Assign a shield") : (lang === "fr" ? "Bouclier assigné" : "Assigned shield"),
      isAvailable ? (lang === "fr" ? "Choisis une unité alliée à protéger." : "Choose an allied unit to protect.") : (lang === "fr" ? "Tu peux retirer ce bouclier et le remettre dans la réserve." : "You can remove this shield and return it to the pool.")
    );
    ...
    if (!isAvailable) {
      ...
      if (assigned) {
        ...
        row.innerHTML = `
          <img class="mkw-resource-portrait" src="${getPortrait(assigned)}" alt="">
          <div class="mkw-resource-info">
            <div class="mkw-resource-name">${getName(assigned, lang)}</div>
            <div class="mkw-resource-class">${getClass(assigned, lang)}</div>
          </div>
          <div class="mkw-resource-value">🛡️</div>
        `;
  ...
  function patchButtons(){
    ...
    refreshKeyButtons();
    refreshShieldButtons();
  }

  function init(){
    patchButtons();
    setTimeout(patchButtons, 250);
    setTimeout(patchButtons, 900);
    setInterval(patchButtons, 2000);
  }
