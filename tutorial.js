(function () {
  ...
  const STEPS = [
    ...
    { target: ".shields-section", titleKey: "shieldTitle", kickerKey: "shieldKicker", textKey: "shieldText", pad: 14 },
    { target: ".repair-section", titleKey: "repairTitle", kickerKey: "repairKicker", textKey: "repairText", pad: 14 },
    { target: "#classActionTitle", titleKey: "classActionTitle", kickerKey: "classActionKicker", textKey: "classActionText", pad: 14 },
    { target: "#ultTitle,#ultToggleContainer", titleKey: "ultimateTitle", kickerKey: "ultimateKicker", textKey: "ultimateText", pad: 14 },
  ...
  function findTarget(step) {
    ...
    if (!el) return null;
    // Remonter à la card parente pour les sections importantes
    if (t.includes("classActionTitle") || t.includes("ultTitle") ||
        t.includes("repair-section")   || t.includes("shields-section")) {
      const card = el.closest(".card") || el.closest(".shields-section") ||
                   el.closest(".repair-section") || el;
  ...
  function updateOverlayPosition() {
    ...
    // Sur mobile : le cadre commence au minimum à 300px (sous le tooltip fixe)
    const top = isMobile() ? Math.max(rawTop, 296) : rawTop;
    if (bottom <= top + 6) {
      // Section pas encore visible, réessayer après un tick
  ...
  function showStep() {
    ...
    renderTooltip(step);
    // Sur mobile : scroller après que le tooltip soit rendu (pour avoir sa hauteur réelle)
