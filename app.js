...
function renderToggleRow(root, toggle, isOn, lang, onChange, sharedShields = null){
  if (toggle.type === 'visual_keys') {
    ...
    keysContainer.style.cssText = `display: flex; gap: 12px; align-items: center; padding: 12px; background: rgba(0,0,0,0.05); border-radius: 8px; margin-bottom: 8px;`;
    ...
    keysDisplay.style.cssText = `display: flex; gap: 8px;`;
    ...
    for (let i = 0; i < maxKeys; i++) {
      ...
      key.style.cssText = `width: 40px; height: 40px; border: 2px solid #ccc; border-radius: 6px; cursor: pointer; background: #f5f5f5; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; padding: 0; background-image: url('./assets/icons/${isShield ? 'shield' : 'key'}_${keyState ? 'on' : 'off'}.svg'); background-size: 70%; background-position: center; background-repeat: no-repeat;`;
...
function initUnitTabs(currentCharId, allChars, lang){
  ...
  if(setup.mode === "single"){
    ...
    if(Array.isArray(draft.activeIds) && draft.activeIds.length){
      tabCharacters = allChars.filter(c => draft.activeIds.includes(c.id) && c.id !== currentCharId && (c.camp || "mechkawaii") === currentCamp);
    }
  }
...
function createCharacterTab(char, lang){
  ...
  infoEl.innerHTML = `<div class="unit-tab-name">${t(char.name, lang)}</div><div class="unit-tab-role">${t(char.class, lang)}</div>`;
