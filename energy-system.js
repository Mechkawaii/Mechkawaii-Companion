(function () {
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      @media (max-width:560px){
        ...
        .mkw-energy-cost-row img, .mkw-energy-action-left img { width:92px; }
      }
    `;
  ...
  function setCurrentEnergy(id, current){ const max = Number(energyData?.maxEnergy || 3); writeJson(getEnergyKey(id), { current: Math.max(0, Math.min(max, Number(current))), max }); }
  ...
  function ensureEnergyStatusNearName(){
    ...
    if(!wrap){ wrap = document.createElement("span"); wrap.id = "mkwEnergyInlineStatus"; wrap.className = "mkw-energy-inline-status"; title.insertAdjacentElement("afterend", wrap); }
    return wrap;
  }
  function updateEnergyStatus(){ const id = currentId(); const wrap = ensureEnergyStatusNearName(); if(!id || !wrap) return; const energy = getCurrentEnergy(id); const max = Number(energyData?.maxEnergy || 3); const src = getAssetFor(energy); wrap.innerHTML = src ? `<img src="${src}" alt="${energy}/${max}">` : `${energy}/${max}`; }
  function clearOldInline(){ document.querySelectorAll(".mkw-energy-cost-row, .mkw-energy-action-line, .mkw-road-toggle-inline").forEach(el => el.remove()); const oldCard = document.querySelector("#mkwEnergyCard"); if(oldCard) oldCard.remove(); }

  function makeSwitch(id, action, cost, labelText){
    const actionState = getActionState(id);
    ...
    label.innerHTML = `<input type="checkbox" ${used ? "checked" : ""}><span class="mkw-energy-slider"></span>${labelText ? `<span class="mkw-energy-switch-label">${labelText}</span>` : ""}`;
    const input = label.querySelector("input");
    input.addEventListener("change", event => {
      if(event.target.checked){ const ok = spendAction(id, action, cost); if(!ok) event.target.checked = false; }
      else { unspendAction(id, action, cost); }
    });
    return label;
  }
  ...
  function costRow(action, cost, opts = {}){
    ...
    left.className = opts.toggle ? "mkw-energy-action-left" : "";
    const src = getAssetFor(cost);
    if(src){ const img = document.createElement("img"); img.src = src; img.alt = `${cost}`; left.appendChild(img); }
    row.appendChild(left);
    if(opts.toggle && Number(cost || 0) > 0) row.appendChild(makeSwitch(id, action, cost, opts.label || tr("useAction")));
    return row;
  }

  function appendCost(selector, action){
    const id = currentId(); const costs = getCostsForId(id); if(!costs) return;
    const cost = Number(costs[action] ?? 0); const container = document.querySelector(selector); if(!container) return;
    container.appendChild(costRow(action, cost));
  }
  ...
  function addRoadToggle(){
    ...
    const body = card?.querySelector(".card-b"); if(!body) return;
    const id = currentId(); const roadState = readJson(getRoadKey(id), { token:getRoundToken(), enabled:false, used:false }); const isActive = roadState.token === getRoundToken() && !!roadState.enabled;
    ...
    sw.innerHTML = `<input type="checkbox" ${isActive ? "checked" : ""}><span class="mkw-energy-slider"></span>`;
  ...
  function render(){
    if(!energyData) return; const id = currentId(); if(!id) return;
    clearOldInline(); updateEnergyStatus();
    ...
    if(moveCardBody) moveCardBody.appendChild(costRow("move", Number(costs.move || 0), { toggle:true, label:tr("move") }));
    addRoadToggle();
    ...
    if(attackCardBody) attackCardBody.appendChild(costRow("ranged_attack", Number(costs.ranged_attack || 0), { toggle:true, label:tr("attack") }));
    const classActionBody = document.querySelector("#classActionBody")?.parentElement;
    if(classActionBody) classActionBody.appendChild(costRow("class_action", Number(costs.class_action || 0), { toggle:true, label:tr("classAction") }));
    const ultBody = document.querySelector("#ultToggleContainer")?.parentElement;
    if(ultBody) ultBody.appendChild(costRow("ultimate", Number(costs.ultimate || 0)));
    bindExistingButtons();
  }

  async function init(){
    ensureStyles(); await loadEnergyData(); const id = currentId(); if(id && !readJson(getEnergyKey(id), null)) resetEnergy(id); render();
    window.addEventListener("mechkawaii:turn-start", event => { const camp = event?.detail?.currentCamp; if(!camp || camp === getCharCamp()){ resetEnergy(currentId()); notify(tr("energyRestored")); } render(); });
